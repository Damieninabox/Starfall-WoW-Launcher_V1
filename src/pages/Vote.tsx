...
export default function Vote() {
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      <header>
        <h1 className="text-2xl font-semibold">Vote for rewards</h1>
        <p className="text-sm text-neutral-400">
          Each active vote site gives you points. Spend them in the Shop.
        </p>
      </header>

      {points && (
        <section className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <Stat label="Vote points" value={points.vote_points} accent="violet" />
          <Stat label="Donation points" value={points.donation_points} accent="amber" />
          <Stat label="Total votes" value={points.total_votes} />
        </section>
      )}
      ...
      {sites.length === 0 ? (
        <div className="rounded border border-dashed border-neutral-800 p-8 text-center text-sm text-neutral-500">
          No vote sites configured. Add rows to <span className="font-mono">starfall_cms.vote_sites</span>.
        </div>
      ) : (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-neutral-500">
                  +{s.pointsReward} points · {s.cooldownHours}h cooldown
                </div>
              </div>
              <button
